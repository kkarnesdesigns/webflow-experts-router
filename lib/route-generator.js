...
class RouteGenerator {
  ...
  /**
   * Generate skill index routes: /{basePath}/{category}/{skill}
   * All experts with that skill (no location filter)
   */
  static generateSkillIndexRoutes(skills, categories, basePath = '/experts') {
    const routes = [];
    ...
    for (const skill of skills) {
      ...
      for (const categoryId of skillCategoryIds) {
        ...
        routes.push({
          path: `${basePath}/${categorySlug}/${skillSlug}`,
          type: 'skill-index',
          params: {
            type: 'skill-index',
            category: categorySlug,
  ...
  /**
   * Generate certification index routes: /{basePath}/{category}/{certification}
   * All experts with that certification (no location filter)
   */
  static generateCertificationIndexRoutes(certifications, categories, basePath = '/experts') {
    const routes = [];
    ...
    for (const certification of certifications) {
      ...
      for (const categoryId of certCategoryIds) {
        ...
        routes.push({
          path: `${basePath}/${categorySlug}/${certSlug}`,
          type: 'certification-index',
          params: {
            type: 'certification-index',
            category: categorySlug,
            certification: certSlug,
  ...
  static generateAllRoutes(data, basePath = '/experts') {
    ...
    let skillIndexRoutes = this.generateSkillIndexRoutes(skills, categories, basePath);
    let certIndexRoutes = this.generateCertificationIndexRoutes(certifications, categories, basePath);
    ...
    if (experts && experts.length > 0) {
      ...
      const beforeCount = {
        skillIndex: skillIndexRoutes.length,
        certIndex: certIndexRoutes.length,
      ...
      // Filter skill index routes (no location filter)
      skillIndexRoutes = skillIndexRoutes.filter(route => {
      ...
      // Filter certification index routes (no location filter)
      certIndexRoutes = certIndexRoutes.filter(route => {
        const count = this.countExpertsForRoute(experts, {
      ...
      console.log(`Filtered skill-index: ${beforeCount.skillIndex} -> ${skillIndexRoutes.length}`);
      console.log(`Filtered cert-index: ${beforeCount.certIndex} -> ${certIndexRoutes.length}`);
    ...
    console.log(`- ${skillIndexRoutes.length} skill index routes (no location)`);
    console.log(`- ${certIndexRoutes.length} certification index routes (no location)`);
  ...
  static createRouteManifest(routes) {
    ...
    for (const route of routes) {
      if (manifest.routes[route.path]) {
        console.warn(`Route collision: ${route.path} (existing: ${manifest.routes[route.path].type}, new: ${route.type})`);
      }
